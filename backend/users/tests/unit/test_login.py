...
class LoginTests(TestCase):
    def setUp(self) -> None:
        ...
        good_user = User.objects.create(**LoginDetails.good_details, username='1')
        good_user.set_password(LoginDetails.good_details['password'])
        good_user.save()
        EmailAddress.objects.create(user=good_user, email=good_user.email, verified=True, primary=True)

        # User with unverified email
        unverified_user = User.objects.create(**LoginDetails.bad_details_email_not_verified, username='2')
        unverified_user.set_password(LoginDetails.bad_details_email_not_verified['password'])
    ...
    def test_login_good_details(self):
        resp = self.client.post('/users/login/', LoginDetails.good_details)
        self.assertEquals(resp.status_code, 200)
        self.assertTrue(resp.json()['key'])

    # Case 2
    def test_login_email_doesnt_exist(self):
        resp = self.client.post('/users/login/', LoginDetails.bad_details_user_email_doesnt_exist)
    ...
    def test_login_wrong_password(self):
        resp = self.client.post('/users/login/', LoginDetails.bad_details_wrong_password)
        self.assertEquals(resp.status_code, 400)
        self.assertDictEqual(
            resp.json(), {'non_field_errors': ['Unable to log in with provided credentials.']}
        )
    ...
    def test_login_unverified_user(self):
        ...
        """
        resp = self.client.post('/users/login/', LoginDetails.bad_details_email_not_verified)
        self.assertEquals(resp.status_code, 400)
        self.assertDictEqual(
    ...
    def test_login_missing_details(self):
        # Email missing
        resp = self.client.post('/users/login/', LoginDetails.bad_detail_email_missing)
        self.assertEquals(resp.status_code, 400)
        self.assertDictEqual(
        ...
        # Password missing
        resp = self.client.post('/users/login/', LoginDetails.bad_details_password_missing)
        self.assertEquals(resp.status_code, 400)
        self.assertDictEqual(
        ...
        # All missing
        resp = self.client.post('/users/login/', LoginDetails.bad_details_no_email_or_password)
        self.assertEquals(resp.status_code, 400)
        self.assertDictEqual(
            resp.json(), {'password': ['This field is required.']}
        )
