...
class AddAccount(TestCase):
    ...
    def test_add_account(self):
        response = self.client.post('/apis/add_account/', data={'name': self.account_name, 'amount': 500},
                                    **self.headers)
        ...
        response = self.client.post('/apis/add_account/', data={'name': self.account_name1},
                                    **self.headers)
        self.assertEquals(response.status_code, 400)
        self.assertEquals(response.json(), {'amount': 'This field is required'})

        response = self.client.post('/apis/add_account/', data={'amount': 500},
                                    **self.headers)
        self.assertEquals(response.status_code, 400)
